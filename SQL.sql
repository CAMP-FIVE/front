...
-- 일반 회원 조회
-- SELECT * FROM MEMBER WHERE ID=? AND STATUS = 'Y' AND ROLE = 'ROLE_USER';

-- 사업자 회원 조회
--SELECT * FROM MEMBER WHERE ID=? AND STATUS = 'Y' AND ROLE = 'ROLE_BUSINESS';

-- 사업자 권한 부여
-- UPDATE MEMBER SET ROLE='ROLE_BUSINESS' WHERE NAME=? AND BUSINESS_NO=?;

-- 멤버 추가
-- NO, 1BUSINESS_NO, 2ID, 3PWD, 4EMAIL, 5NAME, 6PHONE, 7NICK, ENROLL_DATE, MODIFY_DATE, ROLE, STATUS
...
-- 아이디 찾기
-- SELECT ID FROM MEMBER WHERE NAME=? AND STATUS='Y' AND PHONE=?;

-- 비밀번호 찾기
-- SELECT PWD FROM MEMBER WHERE NAME=? AND ID=? AND STATUS='Y' AND PHONE=?;
